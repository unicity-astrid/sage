...
pub(crate) fn load_rules() -> Vec<Rule> {
    let raw = match env::var_opt(POLICY_RULES_ENV) {
        ...
        Err(e) => {
            ...
            log::warn(format!("sage-mcp: policy_rules env read failed: {e:?}"));
            return Vec::new();
        }
    };
    ...
    let parsed: Vec<Rule> = match serde_json::from_str(&raw) {
        ...
        Err(e) => {
            ...
            log::warn(format!("sage-mcp: policy_rules failed to parse: {e}"));
            return Vec::new();
        }
    };
    ...
    if let Err(reason) = validate(&parsed) {
        ...
        log::warn(format!(
            "sage-mcp: policy_rules rejected ({reason}); policy NOT in force"
        ));
        return Vec::new();
    }
    parsed
...
fn audit_load_failure(reason: &str) {
    let _ = ipc::publish_json(
        "sage.v1.audit.policy_load_failed",
