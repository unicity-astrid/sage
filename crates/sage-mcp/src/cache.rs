...
pub(crate) fn upsert(incoming: Vec<McpToolDescriptor>) -> CacheState {
    ...
    for _ in 0..CAS_MAX_RETRIES {
        ...
        let Ok(new_bytes) = serde_json::to_vec(&state) else {
            log::warn("sage-mcp: failed to serialize tool cache");
            return state;
        };
        ...
        match kv::cas(CACHE_KEY, expected.as_deref(), &new_bytes) {
            ...
            Err(e) => {
                log::warn(format!("sage-mcp: tool cache CAS failed: {e}"));
                return state;
            }
        }
    }

    log::warn("sage-mcp: tool cache CAS exhausted retries");
    load()
}
...
pub(crate) fn replace(snapshot: Vec<McpToolDescriptor>) -> CacheState {
    ...
    let expected = kv::get_bytes_opt(CACHE_KEY).ok().flatten();
    let Ok(new_bytes) = serde_json::to_vec(&state) else {
        log::warn("sage-mcp: failed to serialize tool cache (replace)");
        return state;
    };
    match kv::cas(CACHE_KEY, expected.as_deref(), &new_bytes) {
        Ok(true) => state,
        ...
        Err(e) => {
            log::warn(format!("sage-mcp: tool cache CAS (replace) failed: {e}"));
            state
        }
    }
}
