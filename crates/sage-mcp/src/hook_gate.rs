//! Native-tool `before_tool_call` verdict responder — sage-mcp as a
//! `ToolCallBefore` merge participant.
//!
//! This is the SECOND plane of the same per-principal PDP. The broker
//! ([`crate::broker`]) gates the `mcp__sage__*` tool plane in-process and
...
pub(crate) fn handle_before_tool_call(payload: Value) -> Result<(), SysError> {
    let event: BeforeToolCallEvent = match serde_json::from_value(payload) {
        ...
        Err(e) => {
            log::warn(format!(
                "sage-mcp: before_tool_call: malformed event payload: {e}"
            ));
            return Ok(());
        }
    };
    ...
    let Some(reply_topic) = reply_topic(correlation_id) else {
        log::warn(format!(
            "sage-mcp: before_tool_call: rejecting unroutable correlation_id '{correlation_id}'"
        ));
        return Ok(());
    };
    ...
    if let Err(e) = ipc::publish_json(&reply_topic, &reply) {
        log::warn(format!(
            "sage-mcp: before_tool_call: failed to reply on {reply_topic}: {e}"
        ));
    }
    Ok(())
}
...
fn verdict(tool_name: &str, tool_input: &Value) -> Value {
    ...
    if let crate::policy::Decision::Deny { reason } = &decision {
        log::info(format!(
            "sage-mcp: before_tool_call denied native tool '{tool_name}': {reason}"
        ));
